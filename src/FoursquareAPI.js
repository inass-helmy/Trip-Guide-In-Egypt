...
export const getAllVenues = (params) =>
	fetch(`${api}/venues/search?ll=${params.ll}&limit=${auths.limit}&intent=${auths.intent}&radius=${auths.radius}&&categoryId=${categories_id}&query=${params.query}&client_id=${auths.client_id}&client_secret=${auths.client_secret}&v=${auths.version}`)
...
export const getInfoContent= (venueId) =>{
  return   fetch(`${api}/venues/${venueId}?client_id=${auths.client_id}&client_secret=${auths.client_secret}&v=${auths.version}`)
  .then(response => {
	if (!response.ok) {
	  throw response
...
export const getVenuePhoto= (venueId) => {
	return   fetch(`${api}/venues/${venueId}/photos?client_id=${auths.client_id}&client_secret=${auths.client_secret}&v=${auths.version}`)
  .then(response => {
	if (!response.ok) {
	  throw response
	} else  return response.json()
